const imgNode = new Image()
document.body.appendChild(imgNode)



// 源函数
const __loadSrc = function (imgNode, src) {
    imgNode.src = src

}


// 代理函数
const loadSrc = function (imgNode, src) {
    const temp = new Image()
    temp.onload = function () {
        __loadSrc(imgNode, temp.src)// 完成下载
    }

    __loadSrc(
        imgNode,
        'data:image/png;base64,'
    )// 开始下载
    temp.src = src // 开始下载

}

loadSrc(imgNode, 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fqqpublic.qpic.cn%2Fqq_public%2F0%2F0-1905150094-7FB33A3F534DF38F3DC7BFE88617F67C%2F0%3Ffmt%3Djpg%26size%3D51%26h%3D587%26w%3D623%26ppv%3D1&refer=http%3A%2F%2Fqqpublic.qpic.cn&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1621499162&t=bcf7dabbf91e21355b1cbfe92a1f69b7')
